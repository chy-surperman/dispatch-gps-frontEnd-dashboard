...
<script>
	...
	export default{
		name:"statistics-workplan-xxykt",
		data(){
			return {
                activeName:"0",
                tabs:[],
        ...
        created(){
            ...
            tabs[0].label = "湘行一卡通收入统计";
            tabs[1].label = "湘行一卡通客流量统计";
            tabs[2].label = "湘行一卡通收入合计统计";
            tabs[3].label = "湘行一卡通客流量合计统计";
            this.tabs = tabs;
			this._getRouteGroup();
		},
		methods:{
                ...
                statisticsRevenue(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							...
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].routeName) + '_湘行一卡通统计表';
						}
            ...
            _statisticsRouteRevenue(tab,param){
                statisticsRouteRevenue(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].groupName) + '_湘行一卡通统计表';
						}
                        tab.dataTables = resp.result;
                    }else{
                        this.$message.error(resp.define);
