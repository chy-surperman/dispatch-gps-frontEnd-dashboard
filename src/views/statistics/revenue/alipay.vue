        ...
        <el-tabs v-model="activeName" @tab-click="handleTabClick">
            <el-tab-pane v-for="tab in tabs" :key="tab.name" :label="tab.label" :name="tab.name">
                <el-row>
                    ...
                    <!-- <el-table max-height="500" size="small" :data="tab.dataTables" align="center" highlight-current-row style="width: 100%">
                        <el-table-column v-for="dayObj in tab.days" :prop="dayObj.dayStr" :label="dayObj.day" :key="dayObj.id" :width="dayObj.width"></el-table-column>
                    </el-table> -->
                 </el-row>
            </el-tab-pane>
...
<script>
	...
	export default{
		name:"statistics-workplan-merge",
		data(){
			return {
                activeName:"0",
                tabs:[],
        ...
        created(){
            ...
            tabs[0].label = "支付宝收入统计";
            tabs[1].label = "支付宝客流量统计";
            tabs[2].label = "支付宝收入合计统计";
            tabs[3].label = "支付宝客流量合计统计";
            this.tabs = tabs;
			this._getRouteGroup();
		},
		methods:{
                ...
                statisticsRevenue(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							...
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].routeName) + '_支付宝统计表';
						}
            ...
            _statisticsRouteRevenue(tab,param){
                statisticsRouteRevenue(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].groupName) + '_支付宝统计表';
						}
                        tab.dataTables = resp.result;
                    }else{
                        this.$message.error(resp.define);
