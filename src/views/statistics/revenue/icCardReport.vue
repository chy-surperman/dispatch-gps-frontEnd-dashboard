...
<script>
    ...
    import {getReportIcCardRouteNames,statisticsVehicleReportIcCard,statisticsDriverReportIcCard} from '@/api/statistics'
	...
	export default{
        ...
        created(){
            ...
            tabs[0].label = "车辆IC卡上报统计";
            tabs[1].label = "驾驶员IC卡上报统计";
            this.tabs = tabs;
			this._getRouteNames();
		},
		methods:{
			...
			getDatas(name){
				...
				switch(tab.name){
                    case '0':
                        this._statisticsVehicleReportIcCard(tab,tab.model);
                        break;
                    case '1':
                        this._statisticsDriverReportIcCard(tab,tab.model);
                        break;
                }
            },
            _statisticsVehicleReportIcCard(tab,param){
                statisticsVehicleReportIcCard(param).then(resp => {
                   if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].routeName) + '_车辆IC卡上报统计表';
						}
            ...
            _statisticsDriverReportIcCard(tab,param){
                statisticsDriverReportIcCard(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].routeName) + '_驾驶员IC卡上报统计表';
						}
                        tab.dataTables = resp.result;
                    }else{
                        this.$message.error(resp.define);
            ...
            exportData(name){
                ...
                let url = baseURL + "/statistics/income/reportIcCard/download?routeName=" + tab.model.routeName + "&workdate=" + tab.model.workdate;
                let fileName = '';
                switch(tab.name){
                    case '0':
                        url = url + "&type=vehicle";
                        fileName = tab.model.workdate + "_" + tab.model.routeName + "_驾驶员IC卡上报车辆统计表.xlsx";
                        break;
                    case '1':
                        url = url + "&type=driver";
                        fileName = tab.model.workdate + "_" + tab.model.routeName + "_驾驶员IC卡上报驾驶员统计表.xlsx";
                        break;
                }
				axios({
					method:'get',
