	...
	export default{
		...
		methods:{
			...
			getDatas(name){
				...
				switch(tab.name){
                    case '0':
                        this._statisticsCash(tab,tab.model);
                        break;
                    case '1':
                        this._statisticRouteCash(tab,tab.model);
                        break;
                }
            },
            _statisticsCash(tab,param){
                statisticsCash(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].routeName) + '_现金统计表';
						}
            ...
            _statisticRouteCash(tab,param){
                statisticRouteCash(param).then(resp => {
                    if(resp.code == REQ_SUCCESS){
                        for(let i = 0; i < resp.result.length; i++){
							resp.result[i].revenues = this.handlerResult(tab,resp.result[i].revenues);
							resp.result[i].title = this.getTilePrefix(tab,resp.result[i].groupName) + '_现金统计表';
						}
                        tab.dataTables = resp.result;
                    }else{
                        this.$message.error(resp.define);
            ...
            getDownUrl(name,tab){
                ...
                switch(tab.name){
                    case '0':
                        url = url + "cash" + paramsUrl;
                        break;
                    case '1':
                        url = url + "routeCash" + paramsUrl;
                        break;
                }
                return url;
            }
