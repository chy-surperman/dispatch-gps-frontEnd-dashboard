...
<script>
    ...
    import {vehicleWorkplan} from '@/api/statistics'
	...
	export default{
		...
		methods:{
			...
			getWorkplanTrip(){
				vehicleWorkplan(this.select).then(resp => {
					if(resp.code == REQ_SUCCESS){
						for(let i = 0; i < resp.result.length; i++){
							resp.result[i].workplanTrips = this.handlerResult(resp.result[i].workplanTrips);
							resp.result[i].title = this.getTilePrefix(resp.result[i].routeName) + '_车辆趟次统计表';
						}
			...
			exportData(){
				let url = baseURL + "/statistics/workplan/down/vehicle?routeName=" 
								+ this.select.routeName + "&workdate=" 
								+ this.select.workdate + "&groupName=" 
								+ this.select.groupName;

                let fileName = this.getFileNamePrefix() + "_车辆趟次统计表.xlsx";
				axios({
					method:'get',
			        url,
                    responseType:'blob',
