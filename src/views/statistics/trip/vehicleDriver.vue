...
<script>
    ...
    import {driverVehicleWorkplan} from '@/api/statistics'
	...
	export default{
		...
		methods:{
			...
			getWorkplanTrip(){
				driverVehicleWorkplan(this.select).then(resp => {
					if(resp.code == REQ_SUCCESS){
						for(let i = 0; i < resp.result.length; i++){
							resp.result[i].workplanTrips = this.handlerResult(resp.result[i].workplanTrips);
							resp.result[i].title = this.getTilePrefix(resp.result[i].routeName) + '_驾驶员车辆趟次统计表';
						}
			...
			exportData(){
				let url = baseURL + "/statistics/workplan/down/driverVehicle?routeName=" 
								  + this.select.routeName + "&workdate=" 
								  + this.select.workdate + "&groupName=" 
								  + this.select.groupName;
                let fileName = this.getFileNamePrefix() + "_驾驶员车辆趟次统计表.xlsx";
				axios({
					method:'get',
			        url,
                    responseType:'blob',
