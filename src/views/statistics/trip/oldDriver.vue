...
<script>
    ...
    import {driverWorkplan} from '@/api/statistics'
	...
	export default{
		...
		methods:{
			...
			getWorkplanTrip(){
				driverWorkplan(this.select).then(resp => {
					if(resp.code == REQ_SUCCESS){
						for(let i = 0; i < resp.result.length; i++){
							resp.result[i].workplanTrips = this.handlerResult(resp.result[i].workplanTrips);
							resp.result[i].title = this.getTilePrefix(resp.result[i].routeName) + '_驾驶员趟次统计表';
						}
			...
			exportData(){
				let url = baseURL + "/statistics/workplan/down/driver?routeName=" 
